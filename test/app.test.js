...
const { chunk, filterByLevel, makeBlank, isCorrect, matchesIdiom, normalize } = require("../lib.js");
...
test("matchesIdiom は正解と似た意味の別解(accept)を受け入れる", () => {
  const idiom = { phrase: "for example", accept: ["for instance"] };
  assert.ok(matchesIdiom("for example", idiom));
  assert.ok(matchesIdiom("For Instance", idiom)); // 別解＋大文字小文字無視
  assert.ok(!matchesIdiom("for sample", idiom)); // 無関係は不正解
  // accept が無い熟語は完全一致のみ
  assert.ok(matchesIdiom("bite the bullet", { phrase: "bite the bullet" }));
